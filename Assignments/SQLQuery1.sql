...
select * from orders where custid in (select custid from customers where caddress = 'Bangalore')

--6. display list of customer who made lowest purchase (in terms of quantity)
select * from customers where custid in (select custid from orders where quantity = 
...
select * from orders where day(orderdate) < 16 and custid in (select custid from customers where caddress!= 'Bangalore')

--11. Display list of customers  from delhi and Bangalore who made purchase of less than 3 product  
select * from customers where caddress in ('Delhi','Bangalore') and custid in (select custid from orders where quantity < 3)
...
update orders set price = price * 1.10 where product='books' and custid in (select custid from customers where caddress='Bangalore')

--14.Display orderdetails in following format 
--OrderID-Date Total(price*qty) 
--100-1/1/2000 500
select concat(orderid, '-',convert(varchar,orderdate, 101)) as [OrderID-Date], (price*quantity) as [Total(price*qty)] from orders
